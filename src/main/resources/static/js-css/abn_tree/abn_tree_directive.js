(function() {
  ...
  module.directive('abnTree', [
    '$timeout','MyService', function($timeout,MyService) {
      return {
        restrict: 'E',
        ...
        link: function(scope, element, attrs) {
		  ...
		  scope.$on('$routeChangeStart', function(next, current) {	  
			if(!(angular.isUndefined(current.$$route))){
				var pathSet	= current.$$route.originalPath;
				var data=scope.treeData;
				var setSelectedBranch=false;
				for(var i=0;i<data.length;i++){
					var parents=data[i];
					var childrens=parents.children
					for(var j=0;j<childrens.length;j++){
						if(childrens[j].data.path == pathSet){
								childrens[j].selected=true;
								if(!setSelectedBranch){
									selected_branch=parents;
									setSelectedBranch=true;
								}
						}else{
							childrens[j].selected=false;
						}
					}
				}
			}	
		  ...
		  scope.getParentBranch=function(data,pathSet){
			  var parentBranch='';
			  for(var i=0;i<data.length;i++){
					var parents=data[i];
					var childrens=parents.children
					for(var j=0;j<childrens.length;j++){
						if((!(angular.isUndefined(childrens[j].selected))) && childrens[j].selected){
								if(childrens[j].data.path != pathSet){
									return parents;
								}
						}
          ...
          if (attrs.initialSelection != null) {
            for_each_branch(function(b) {
              if (b.leafIndex == attrs.initialSelection) {
                return $timeout(function() {
          ...
          for_each_branch(function(b, level) {
            b.level = level;
			if(angular.isUndefined(b.expanded)){
